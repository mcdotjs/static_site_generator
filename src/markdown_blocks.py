...
def markdown_to_html_node_Mirko_nestaci__(markdown):
    ...
    nodes = []
    for block in markdown_in_blocks:
        block_type = block_to_block_type(block)
        tag = tags[block_type]
        if block_type == "heading":
            kkk = block.split()[0]
            tag = tags["heading"][len(kkk)-1]
        if block_type == "code":
            tag = "pre"
        if block_type == "quto":
            tag = "blockquote"
        ...
        for i in range(1, len(children)):
            html_children.append(text_node_to_html_node(children[i]))

        new_html_node = HTMLNode(
            tag, children[0].text, children=html_children)
